...
void deletePlayer()
{
	...
	if(key >= 0 && key < players_array_size)
	{
		if(Player[key].ID != -1)
		{
			printf(" ID: %d \n Name: %s \n Whight: %f.2 \n Hight: %f.2 \n Deffence: %f % \n Number Of Goals: %d \n Rank: %d \n Club: %s\n",Player[key].ID , Player[key].name , Player[key].whight , Player[key].hight , Player[key].deffence , Player[key].num_Goals , Player[key].rank ,Club[Player[key].club_ID-1].name);
...
void deleteClub()
{
	...
	if(key >= 0 && key < clubs_array_size)
	{
		if(Club[key].ID != -1)
		{
			printf(" ID: %d \n Name: %s \n Number Of Goals: %d \n Rank: %f\n" , Club[key].ID , Club[key].name , Club[key].num_Goals , Club[key].rank);
			printf(" ~~~~~~~~ Team Members ~~~~~~~~\n\n");
