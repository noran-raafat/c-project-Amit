...
/* First menu in search */

void displayMenu()
{
	calcStats();  /* function that calculates stats for clubs*/
...
void displayAllPlayer()
{
	...
	int c,i;
	i=0;
	for( c = 0 ; c < PLAYERS_ARRAY_SIZE ; c++ )  /* loops over all players */
	{
		if( Player[c].ID != -1 )   /* player exists*/
		{
			...
			printf("\n~~~~~~~~~~~~~~~~~~~~~\n");
			i++;  /* flag for players found */
			
		}
	}
	if( i != 0 ) /* found players*/
	{
...
void displayAllClub()
{
	...
	char c,i,g,x=0;
	calcStats();  
	for( c = 0 ; c < CLUBS_ARRAY_SIZE ; c++ )  /*Loop over all clubs*/
	{
		if(Club[c].ID != -1)
		{
			printf("--------------------"); 
			...
			printf(" ~~~~~~~~ Team Members ~~~~~~~~\n\n");
			for( i = 0 ; i < CLUB_PLAYERS ; i++) /*Loop over players of club  */
			{
...
int searchPlayerName()
{
	...
	scanf("%s",&KEY);
	for(p = 0 ; p < PLAYERS_ARRAY_SIZE ; p++) /* Loop over all players */
	{
		while(KEY[c] != '\0') 
		{
			
			if(Player[p].name[c] == KEY[c])  /* Check each character */
			{
		...
		if(found == c) /*All characters are the same */
		{
			return p;
		}
